...
sender1 = net.addDocker('h1', ip='192.168.3.111', dimage='pyrtc_image:latest', privileged=True, cap_add=['NET_ADMIN'], security_opt=['apparmor=unconfined', 'seccomp=unconfined'])
sender2 = net.addDocker('h2', ip='192.168.3.112', dimage='pyrtc_image:latest', privileged=True, cap_add=['NET_ADMIN'], security_opt=['apparmor=unconfined', 'seccomp=unconfined'])
receiver1 = net.addDocker('h3', ip='192.168.3.113', dimage='pyrtc_image:latest', privileged=True, cap_add=['NET_ADMIN'], security_opt=['apparmor=unconfined', 'seccomp=unconfined'])
receiver2 = net.addDocker('h4', ip='192.168.3.114', dimage='pyrtc_image:latest', privileged=True, cap_add=['NET_ADMIN'], security_opt=['apparmor=unconfined', 'seccomp=unconfined'])
